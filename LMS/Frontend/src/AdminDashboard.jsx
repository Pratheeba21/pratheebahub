...
export default function AdminDashboard({
  ...
  const [subjectSearch, setSubjectSearch] = useState("");
  const [editingSubject, setEditingSubject] = useState(null); // { _id, name, coverImage }
const [newSubjectCoverImage, setNewSubjectCoverImage] = useState("");
  const [subjectModalStudent, setSubjectModalStudent] = useState(null); // student object for modal
  const [subjectModalDropdown, setSubjectModalDropdown] = useState(""); // selected subject id in dropdown
  ...
  const [newCourseName, setNewCourseName] = useState("");
  const [newCourseCoverImage, setNewCourseCoverImage] = useState("");
  const [newModuleTitle, setNewModuleTitle] = useState("");
  const [newTopicTitle, setNewTopicTitle] = useState("");
  const [showCreateModule, setShowCreateModule] = useState(false);
  const [showCreateTopic, setShowCreateTopic] = useState(false);

  // Materials / Lab Practicals share one form (same { title, htmlContent } shape)
  const [courseContentForm, setCourseContentForm] = useState({
    title: "",
    html: "",
  });
  const [courseContentFormType, setCourseContentFormType] =
    useState("materials");
  const [showCreateCourseContent, setShowCreateCourseContent] = useState(false);

  // Course quiz creation
  const [courseQuizTitle, setCourseQuizTitle] = useState("");
  const [courseQuizQuestionCount, setCourseQuizQuestionCount] = useState(1);
  const [courseQuizQuestions, setCourseQuizQuestions] = useState([
    { questionText: "", options: ["", "", "", ""], correctIndex: 0 },
  ]);
  const [showCreateCourseQuiz, setShowCreateCourseQuiz] = useState(false);

  // Admin course-quiz preview (read-only, ephemeral)
  const [previewCourseQuiz, setPreviewCourseQuiz] = useState(null);
  const [previewCourseQuizAnswers, setPreviewCourseQuizAnswers] = useState([]);
  const [previewCourseQuizSubmitted, setPreviewCourseQuizSubmitted] =
    useState(false);
  ...
  const [editingCourse, setEditingCourse] = useState(null); // { _id, name, coverImage }
  ...
  //     const created = newCourseName;
  //     try {
  //       await axios.post(`${API_BASE}/courses`, { name: created });
  //       setNewCourseName("");
  //       fetchCourses();
  //       await showAlert(
  //         "Course Created",
  ...
  //     } catch (err) {
  //       console.error(err);
  //       await showAlert("Error", "Failed to create course.", "danger");
  //     }
  //   };
  ...
  const handleCreateCourse = async (e) => {
    ...
    try {
      await axios.post(`${API_BASE}/courses`, {
        name: created,
        coverImage: newCourseCoverImage,
      });
      setNewCourseName("");
      setNewCourseCoverImage("");
      fetchCourses();
      await showAlert(
        "Course Created",
        `"${created}" has been added successfully!`,
  ...
  const handleEditCourse = async (e) => {
    ...
    try {
      await axios.put(`${API_BASE}/courses/${editingCourse._id}`, {
        name: editingCourse.name,
        coverImage: editingCourse.coverImage,
      });
      setEditingCourse(null);
      fetchCourses();
      await showAlert("Updated", "Course updated successfully!", "success");
    } catch (err) {
      console.error(err);
      await showAlert("Error", "Failed to update course.", "danger");
    }
  };
  ...
  const handleDeleteCourse = async (e, courseId, courseName) => {
    ...
    const confirmed = await showConfirm(
      "Delete Course",
      `Permanently delete "${courseName}" and all its modules, topics, and content? This cannot be undone.`,
      "danger",
    );
    if (!confirmed) return;
    try {
      ...
      await showAlert(
        "Course Deleted",
        `"${courseName}" has been permanently removed.`,
        "danger",
      );
    } catch (err) {
      console.error(err);
      await showAlert("Error", "Failed to delete course.", "danger");
    }
  };
  ...
  const fetchCourseRequests = async () => {
    ...
    try {
      const res = await axios.get(`${API_BASE}/course-requests`);
  ...
  const handleCourseRequestAction = async (requestId, action) => {
    try {
      await axios.put(`${API_BASE}/course-requests/${requestId}`, { action });
      await showAlert(
        action === "approve" ? "Access Approved" : "Request Rejected",
        action === "approve"
          ? "Student has been granted access to the course."
          : "Student's request has been rejected.",
        action === "approve" ? "success" : "danger",
      );
      fetchCourseRequests();
    } catch (err) {
  ...
  const handleCreateModule = async (e) => {
    ...
    try {
      await axios.post(`${API_BASE}/courses/${activeCourseId}/modules`, {
  ...
  const handleDeleteModule = async (e, moduleId, moduleTitle) => {
    e.stopPropagation();
    const confirmed = await showConfirm(
      "Delete Module",
      `Delete "${moduleTitle}" and all its topics? This cannot be undone.`,
      "danger",
    );
    if (!confirmed) return;
    try {
      await axios.delete(
        `${API_BASE}/courses/${activeCourseId}/modules/${moduleId}`,
      );
  ...
  const handleCreateTopic = async (e) => {
    e.preventDefault();
    if (!activeCourseId || !activeModuleId || !newTopicTitle) return;
    try {
      await axios.post(
        `${API_BASE}/courses/${activeCourseId}/modules/${activeModuleId}/topics`,
        { title: newTopicTitle },
      );
      setNewTopicTitle("");
  ...
  const handleDeleteTopic = async (e, topicId, topicTitle) => {
    e.stopPropagation();
    const confirmed = await showConfirm(
      "Delete Topic",
      `Delete "${topicTitle}" and all its content? This cannot be undone.`,
      "danger",
    );
    if (!confirmed) return;
    try {
      await axios.delete(
        `${API_BASE}/courses/${activeCourseId}/modules/${activeModuleId}/topics/${topicId}`,
      );
  ...
  const handleCreateCourseContent = async (e) => {
    e.preventDefault();
    if (
      !activeCourseId ||
      !activeModuleId ||
      !activeTopicId ||
      !courseContentForm.title ||
      !courseContentForm.html
    )
      return;
    try {
      await axios.post(
        `${API_BASE}/courses/${activeCourseId}/modules/${activeModuleId}/topics/${activeTopicId}/content`,
        {
          type: courseContentFormType,
          title: courseContentForm.title,
          htmlContent: courseContentForm.html,
        },
      );
      setCourseContentForm({ title: "", html: "" });
      setShowCreateCourseContent(false);
      fetchCourses();
      await showAlert(
        "Upload Complete",
        `${courseContentFormType === "materials" ? "Material" : "Lab Practical"} uploaded successfully!`,
        "success",
      );
    } catch (err) {
      console.error(err);
  ...
  const handleCourseQuizQuestionCountChange = (count) => {
    const val = parseInt(count) || 1;
    setCourseQuizQuestionCount(val);
    const base = [...courseQuizQuestions];
    while (base.length < val)
      base.push({
        questionText: "",
        options: ["", "", "", ""],
    ...
    base.splice(val);
    setCourseQuizQuestions(base);
  };

  const updateCourseQuizQuestionField = (index, key, value) => {
    const updated = [...courseQuizQuestions];
    updated[index][key] = value;
    setCourseQuizQuestions(updated);
  };

  const updateCourseQuizOption = (qIdx, optIdx, value) => {
    const updated = [...courseQuizQuestions];
    updated[qIdx].options[optIdx] = value;
    setCourseQuizQuestions(updated);
  };

  const handleCreateCourseQuiz = async (e) => {
    e.preventDefault();
    if (
      !activeCourseId ||
      !activeModuleId ||
    ...
    try {
      ...
      setCourseQuizTitle("");
      setCourseQuizQuestionCount(1);
      setCourseQuizQuestions([
        { questionText: "", options: ["", "", "", ""], correctIndex: 0 },
      ]);
      setShowCreateCourseQuiz(false);
      fetchCourses();
      await showAlert("Quiz Created", "Quiz created successfully!", "success");
    } catch (err) {
      console.error(err);
  ...
  const handleDeleteCourseContent = async (e, type, contentId) => {
    e.stopPropagation();
    const confirmed = await showConfirm(
      "Delete Content",
    ...
    try {
      await axios.delete(
        `${API_BASE}/courses/${activeCourseId}/modules/${activeModuleId}/topics/${activeTopicId}/content/${type}/${contentId}`,
      );
      fetchCourses();
    } catch (err) {
      console.error(err);
  ...
  const launchAdminCourseQuizPreview = (quiz) => {
    setPreviewCourseQuiz(quiz);
    setPreviewCourseQuizAnswers(
      new Array((quiz.questions || []).length).fill(null),
    );
    setPreviewCourseQuizSubmitted(false);
  };
  ...
  const handleCreateSubject = async (e) => {
    ...
    await axios.post(`${API_BASE}/subjects`, {
      name: newSubjectName,
      coverImage: newSubjectCoverImage,
    });
    const created = newSubjectName;
    setNewSubjectName("");
    setNewSubjectCoverImage("");
      fetchAdminData();
      await showAlert(
        "Subject Created",
        `"${created}" has been added successfully!`,
    ...
    <div>
      ...
      {showCreateSubjectModal && (
        <div
          ...
          onClick={(e) => {
            if (e.target === e.currentTarget) setShowCreateSubjectModal(false);
          }}>
          <div
            style={{
              background: "var(--surface)",
            ...
            <form
              ...
              <input
                ...
                className="input-field"
                placeholder="https://..."
                value={newSubjectCoverImage}
                onChange={(e) => setNewSubjectCoverImage(e.target.value)}
              />
              {newSubjectCoverImage && (
                <div style={{ marginBottom: "0.75rem" }}>
                  <div
                    style={{
                      fontFamily: "JetBrains Mono",
    ...
    <div>
      ...
      {showCreateStudentModal && (
        <div
          ...
          onClick={(e) => {
            if (e.target === e.currentTarget) setShowCreateStudentModal(false);
          }}>
          <div
            style={{
              background: "var(--surface)",
  ...
  const renderControlOverview = () => (
    <div>
      ...
      <div
        ...
        {/* <div className="form-panel">
          <div className="form-title">🎓 Add New Course</div>
          <form onSubmit={handleCreateCourse}>
            <input
              type="text"
              className="input-field"
              placeholder="Course name (e.g., AI & Deep Learning Foundations)"
              value={newCourseName}
              onChange={(e) => setNewCourseName(e.target.value)}
            />
            <button type="submit" className="action-btn">
              Instantiate Course Core
            </button>
          </form>
        </div> */}
        ...
        <div className="form-panel">
          ...
          <form onSubmit={handleCreateCourse}>
            ...
            <input
              ...
              value={newCourseCoverImage}
              onChange={(e) => setNewCourseCoverImage(e.target.value)}
            />
            <button type="submit" className="action-btn">
              Instantiate Course Core
            </button>
  ...
  //     <div>
  //       <div className="hero">
  //         <div className="hero-badge">Course Registry</div>
  //         <h1>All Courses</h1>
  //       </div>
  //       <div
  //         style={{
  ...
  //             color: "var(--blue)",
  //           }}>
  //           TOTAL: <b style={{ color: "var(--text)" }}>{courses.length}</b>
  //         </div>
  //       </div>
  ...
  //             fontSize: "0.85rem",
  //           }}>
  //           No courses created yet. Add one from the Control Overview.
  //         </div>
  //       ) : (
  //         <div
  //           style={{
  ...
  //                   }}
  //                   onClick={(e) =>
  //                     handleDeleteCourse(e, course._id, course.name)
  //                   }>
  //                   🗑
  //                 </button>
  //               </div>
          ...
          {courses.map((course) => (
            <div key={course._id}>
              <div
                style={{
                  background: "var(--surface)",
                  border: `1px solid ${editingCourse?._id === course._id ? "var(--blue)" : "var(--border)"}`,
                  borderRadius: "14px",
                  overflow: "hidden",
                  transition: "border-color 0.2s, transform 0.2s",
                }}
    ...
    <div>
      ...
      <div
        ...
        <div
          ...
          PENDING:{" "}
          <b style={{ color: "var(--text)" }}>{courseRequests.length}</b>
        </div>
      </div>
      ...
      {courseRequestsLoading ? (
        ...
        <div
          ...
          <div style={{ fontSize: "2.5rem", marginBottom: "1rem" }}>📭</div>
          No pending course access requests at this time.
        </div>
      ) : (
        <div
          style={{
                  ...
                  <td style={{ padding: "14px 16px" }}>
                    <div style={{ display: "flex", gap: "8px" }}>
                      <button
                        ...
                        onClick={() =>
                          handleCourseRequestAction(req._id, "approve")
                        }>
                        ✓ Approve
                      </button>
                      <button
                        ...
                        onClick={() =>
                          handleCourseRequestAction(req._id, "reject")
                        }>
                        ✕ Reject
                      </button>
                    </div>
  ...
  return (
    <div className="dashboard-container">
      ...
      <div className={`sidebar ${sidebarOpen ? "open" : "closed"}`}>
        ...
        <div
          className={`sidebar-item ${activeView === "courses" ? "active" : ""}`}
          onClick={() => {
            setActiveSubjectId(null);
            setActiveCourseId(null);
            setActiveTab(null);
        ...
        <div
          className={`sidebar-item ${activeView === "courseRequests" ? "active" : ""}`}
          onClick={() => {
            setActiveSubjectId(null);
            setActiveCourseId(null);
            setActiveTab(null);
            setActiveHtmlContent(null);
            setActiveView("courseRequests");
            fetchCourseRequests();
          }}>
          <span style={{ fontSize: "1rem" }}>📬</span> Course Requests
          {courseRequests.length > 0 && (
            <span
              style={{
                marginLeft: "auto",
        ...
        {courses.map((course) => (
          ...
          <div
            ...
            <span style={{ fontSize: "1rem" }}>📬</span> Subjects Requests
            {subjectRequests.length > 0 && (
              <span
                style={{
                  marginLeft: "auto",
      ...
      <div
        ...
        {selectedSubject && activeView !== "progress" ? (
          ...
          <div>
              ...
              {activeTopicContentType && (
                <>
                  ...
                  <span style={{ color: "var(--text)" }}>
                    {activeTopicContentType === "materials"
                      ? "📘 Materials"
                      : activeTopicContentType === "quizzes"
                        ? "🎯 Quizzes"
                        : "🧪 Lab Practicals"}
                  </span>
                </>
                    ...
                    {selectedCourse.modules.map((mod) => (
                      <div
                        ...
                        <span
                          ...
                          📑 {mod.topics?.length || 0} topic
                          {(mod.topics?.length || 0) !== 1 ? "s" : ""}
                        </span>
                        <div
                          style={{
                            display: "flex",
                          ...
                          <button
                            ...
                            onClick={(e) =>
                              handleDeleteModule(e, mod._id, mod.title)
                            }>
                            🗑
                          </button>
                        </div>
                      ...
                      <div
                        ...
                        <div
                          ...
                          <button
                            ...
                            onClick={(e) =>
                              handleDeleteTopic(e, topic._id, topic.title)
                            }>
                            🗑
                          </button>
                        </div>
            ...
            {/* LEVEL 3: Topic workspace — the 3 cards */}
            {activeTopicId &&
              !activeTopicContentType &&
                ...
                <div>
                  ...
                  <div className="content-grid-cards">
                    <div
                      className="interactive-card"
                      style={{ "--accent-color": "var(--green)" }}
                      onClick={() => setActiveTopicContentType("materials")}>
                      <div className="interactive-card-icon">📘</div>
                      <div
                        className="interactive-card-title"
                        style={{ color: "var(--green)" }}>
                      ...
                      <div className="interactive-card-desc">
                        Reference notes and concept write-ups.
                      </div>
                    </div>
                    <div
                      className="interactive-card"
                      style={{ "--accent-color": "var(--pink)" }}
                      onClick={() => setActiveTopicContentType("quizzes")}>
                      <div className="interactive-card-icon">🎯</div>
                      <div
                        className="interactive-card-title"
                        style={{ color: "var(--pink)" }}>
                      ...
                      <div className="interactive-card-desc">
                        Knowledge checks for this topic.
                      </div>
                    </div>
                    <div
                      className="interactive-card"
            ...
            {/* LEVEL 4: content list for the selected card */}
            {activeTopicContentType &&
              !activeCourseHtmlContent &&
              !previewCourseQuiz &&
              (() => {
                const items = selectedTopic?.[activeTopicContentType] || [];
                const meta = {
                  materials: {
                    icon: "📘",
                    label: "Materials",
                ...
                return (
                  <div>
                    <div
                      ...
                      <h2 style={{ fontFamily: "JetBrains Mono", margin: 0 }}>
                      ...
                      <div style={{ position: "absolute", right: 0 }}>
                        {activeTopicContentType === "quizzes" ? (
                          <button
                            ...
                            onClick={() => setShowCreateCourseQuiz(true)}>
                            ➕ New Quiz
                          </button>
                        ) : (
                          <button
                            ...
                            onClick={() => {
                              setCourseContentFormType(activeTopicContentType);
                              setShowCreateCourseContent(true);
                            }}>
                            ➕ New{" "}
                            {activeTopicContentType === "materials"
                              ? "Material"
                          ...
                          <span
                            ...
                            onClick={() => {
                              if (activeTopicContentType === "quizzes")
                                launchAdminCourseQuizPreview(item);
                              else setActiveCourseHtmlContent(item.htmlContent);
                            }}>
                            ⚡ {item.title}
                          </span>
                          <div style={{ display: "flex", gap: "10px" }}>
                            <button
                              ...
                              onClick={() => {
                                if (activeTopicContentType === "quizzes")
                                  ...
                                  setActiveCourseHtmlContent(item.htmlContent);
                              }}>
                              View
                            </button>
                            <button
            ...
            {/* Viewer: Material / Lab Practical HTML */}
            {activeCourseHtmlContent && (
              <div>
                <button
                  className="action-btn close-view-btn"
                ...
                <iframe
                  srcDoc={activeCourseHtmlContent}
                  title="Content Preview"
                  style={{
                    width: "100%",
                    minHeight: "85vh",
            ...
            {previewCourseQuiz && (
              <div
                ...
                <div className="quiz-hero">
                  <div className="quiz-badge">👁️ Admin Preview</div>
                  <div className="quiz-title">{previewCourseQuiz.title}</div>
                  <div className="quiz-sub">
                    {(previewCourseQuiz.questions || []).length} questions ·
                    preview mode
                  </div>
                </div>
                {!previewCourseQuizSubmitted ? (
                  <>
                    {(previewCourseQuiz.questions || []).map((q, i) => (
                      <div
                        className="q-card"
                        key={i}
                        style={{
                          borderLeft:
                            previewCourseQuizAnswers[i] !== null
                              ? "3px solid var(--blue)"
                              : "1px solid var(--border)",
                        }}>
                        <div className="q-num">
                        ...
                        <div className="options">
                          {(q.options || []).map((opt, j) => (
                            <button
                              key={j}
                              className={`opt ${previewCourseQuizAnswers[i] === j ? "selected" : ""}`}
                              onClick={() => {
                                const upd = [...previewCourseQuizAnswers];
                                upd[i] = j;
                                setPreviewCourseQuizAnswers(upd);
                              }}>
                              <div className="opt-circle">
                                {String.fromCharCode(65 + j)}
                              </div>
                    ...
                    <button
                      className="submit-btn"
                      style={{
                        background: previewCourseQuizAnswers.every(
                          (a) => a !== null,
                        )
                          ? "var(--blue)"
                          : "transparent",
                      }}
                      disabled={previewCourseQuizAnswers.some(
                        (a) => a === null,
                      )}
                      onClick={() => setPreviewCourseQuizSubmitted(true)}>
                      Submit Preview →
                    </button>
                  </>
                ) : (
                  <div className="result-panel" style={{ display: "block" }}>
                    {(() => {
                      const correct = (
                        previewCourseQuiz.questions || []
                      ).filter(
                        (q, i) =>
                          previewCourseQuizAnswers[i] === q.correctIndex,
                      ).length;
                      const total = previewCourseQuiz.questions.length;
                      const pct = Math.round((correct / total) * 100);
                      return (
                        <>
                          <div className="result-score">
                          ...
                          <div className="stat-row">
                            <div className="stat">
                              <div className="stat-val s-green">{correct}</div>
                              <div className="stat-lab">Correct</div>
                            </div>
                            <div className="stat">
                              <div className="stat-val s-red">
            ...
            {showCreateModule && (
              <div
                ...
                onClick={(e) => {
                  if (e.target === e.currentTarget) setShowCreateModule(false);
                }}>
                <div
                  style={{
                    background: "var(--surface)",
                  ...
                  <div className="form-title" style={{ color: "var(--amber)" }}>
                    ➕ New Module
                  </div>
                  <form onSubmit={handleCreateModule}>
                    <input
                      type="text"
                      className="input-field"
                      placeholder="Module title (e.g., Module 1: Foundations of AI)"
            ...
            {showCreateTopic && (
              <div
                ...
                onClick={(e) => {
                  if (e.target === e.currentTarget) setShowCreateTopic(false);
                }}>
                <div
                  style={{
                    background: "var(--surface)",
                  ...
                  <div className="form-title" style={{ color: "var(--pink)" }}>
                    ➕ New Topic
                  </div>
                  <form onSubmit={handleCreateTopic}>
                    <input
                      type="text"
                      className="input-field"
                      placeholder="Topic title (e.g., Landscape of Artificial Intelligence)"
                      value={newTopicTitle}
            ...
            {/* ── Modal: New Material / Lab Practical (shared form) ── */}
            {showCreateCourseContent && (
              <div
                ...
                <div
                  ...
                  <div className="form-title" style={{ color: "var(--green)" }}>
                    ➕ New{" "}
                    {courseContentFormType === "materials"
                      ? "📘 Material"
                      : "🧪 Lab Practical"}
                  </div>
                  <form onSubmit={handleCreateCourseContent}>
                    <input
                      type="text"
                      className="input-field"
                      placeholder="Title"
                      value={courseContentForm.title}
                      onChange={(e) =>
                        setCourseContentForm({
                          ...courseContentForm,
                          title: e.target.value,
                        })
                      }
                    ...
                    <textarea
                      ...
                      placeholder="Paste full raw HTML content (e.g. notes, <pre><code> walkthroughs)..."
                      value={courseContentForm.html}
                      onChange={(e) =>
                        setCourseContentForm({
                          ...courseContentForm,
                          html: e.target.value,
                    ...
                    <div
                      ...
                      <button
                        ...
                        onClick={() => setShowCreateCourseContent(false)}>
                        Cancel
                      </button>
                    </div>
                  </form>
            ...
            {/* ── Modal: New Course Quiz ── */}
            {showCreateCourseQuiz && (
              <div
                style={{
                  position: "fixed",
                  inset: 0,
                ...
                <div
                  ...
                  <form onSubmit={handleCreateCourseQuiz}>
                    <input
                      type="text"
                      className="input-field"
                      placeholder="Quiz Title"
                    ...
                    <div
                      ...
                      <input
                        ...
                        className="input-field"
                        style={{ margin: 0, maxWidth: "100px" }}
                        value={courseQuizQuestionCount}
                        onChange={(e) =>
                          handleCourseQuizQuestionCountChange(e.target.value)
                        }
                      />
                    </div>
                    {courseQuizQuestions.map((q, qi) => (
                      <div
                        key={qi}
                        style={{
                          borderLeft: "2px solid var(--pink)",
                          ...
                          paddingBottom: "1rem",
                          borderBottom:
                            qi < courseQuizQuestions.length - 1
                              ? "1px solid var(--border)"
                              : "none",
                        }}>
                        <div
                        ...
                        {q.options.map((opt, oi) => (
                          <div
                            ...
                            <input
                              ...
                              value={opt}
                              onChange={(e) =>
                                updateCourseQuizOption(qi, oi, e.target.value)
                              }
                            />
                            {q.correctIndex === oi && (
                              <span
                    ...
                    <div style={{ display: "flex", gap: "10px" }}>
                      ...
                      <button
                        ...
                        onClick={() => setShowCreateCourseQuiz(false)}>
                        Cancel
                      </button>
                    </div>
                  </form>
