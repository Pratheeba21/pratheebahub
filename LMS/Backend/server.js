...
//   "https://pratheebahub.onrender.com",
//   "http://localhost:5173",
// ];
...
const TopicSchema = new mongoose.Schema({
  ...
  materials: [{ title: String, htmlContent: String }],
  labPracticals: [{ title: String, htmlContent: String }],
  quizzes: [
    {
      title: String,
      questions: [
...
app.delete("/api/courses/:courseId", async (req, res) => {
  try {
    ...
    await User.updateMany(
      { assignedCourses: req.params.courseId },
      { $pull: { assignedCourses: req.params.courseId } },
    );
    await CourseRequest.deleteMany({ courseId: req.params.courseId });
...
// ── TOPIC ROUTE CONTROLLERS ─────────────────────────────────────────────────

app.post(
  "/api/courses/:courseId/modules/:moduleId/topics",
  async (req, res) => {
    try {
      const { title, description } = req.body;
      const course = await Course.findById(req.params.courseId);
...
// ── TOPIC CONTENT ROUTE CONTROLLERS (materials / labPracticals / quizzes) ──

app.post(
  "/api/courses/:courseId/modules/:moduleId/topics/:topicId/content",
  async (req, res) => {
    try {
      const { type, title, htmlContent, questions } = req.body;
      if (!["materials", "labPracticals", "quizzes"].includes(type)) {
        return res.status(400).json({ error: "Invalid content type." });
      }
      const course = await Course.findById(req.params.courseId);
      if (!course) return res.status(404).json({ error: "Course not found." });
      ...
      const newItem =
        type === "quizzes"
          ...
          : { title, htmlContent: htmlContent || "" };

      topic[type].push(newItem);
      await course.save();
      res.json(course);
    } catch (err) {
      res.status(400).json({ error: err.message });
...
app.delete(
  "/api/courses/:courseId/modules/:moduleId/topics/:topicId/content/:type/:contentId",
  async (req, res) => {
    try {
      const { courseId, moduleId, topicId, type, contentId } = req.params;
      if (!["materials", "labPracticals", "quizzes"].includes(type)) {
        return res.status(400).json({ error: "Invalid content type." });
      }
...
app.post("/api/course-requests", async (req, res) => {
  try {
    ...
    if (
      ...
      return res.status(400).json({ error: "Already assigned to this course." });
    ...
    if (existing)
      return res.status(400).json({ error: "Request already pending." });
    const request = new CourseRequest({ username, email: user.email, courseId });
    await request.save();
    res.status(201).json(request);
  } catch (err) {
    res.status(400).json({ error: err.message });
...
app.get("/api/course-requests", async (req, res) => {
  try {
    const requests = await CourseRequest.find({ status: "pending" }).populate(
      "courseId",
      "name",
    );
    res.json(requests);
...
// Approve or reject a course request (admin)
app.put("/api/course-requests/:id", async (req, res) => {
  try {
    const { action } = req.body; // "approve" | "reject"
    const request = await CourseRequest.findById(req.params.id).populate(
      "courseId",
      "name",
    );
    if (!request) return res.status(404).json({ error: "Request not found." });
    ...
    if (action === "approve") {
      await User.findOneAndUpdate(
        { username: request.username },
        { $addToSet: { assignedCourses: request.courseId._id } },
      );
    }
...
app.get("/api/course-requests/student/:username", async (req, res) => {
  try {
